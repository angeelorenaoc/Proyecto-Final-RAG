...
void initUHD(SSOLED *oled){
  oledSetBackBuffer(oled, ucBuffer);
  oledDrawLine(oled,0,2,125,2,1);
  oledDrawLine(oled,0,63,125,63,1);
  oledDrawLine(oled,0,2,0,63,1);
  oledDrawLine(oled,125,2,125,63,1);
  oledWriteString(oled, 0,8,1,(char *)"Rectangulo 1", FONT_6x8, 0, 1);
  //sleep_ms(1000);
}
void testOLED(SSOLED *oled){
  oledFill(&oled, 0,1);
